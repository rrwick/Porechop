...
def main():
    ...
    matching_sets = find_matching_adapter_sets(
        check_reads,
        args.verbosity,
        args.end_size,
    ...
    if args.barcode_dir:
        forward_or_reverse_barcodes = choose_barcoding_kit(
            matching_sets, args.verbosity, args.print_dest
        )
    else:
    ...
    if matching_sets:
        ...
        if not args.no_split:
            ...
            display_read_middle_trimming_summary(
                reads, args.discard_middle, args.verbosity, args.print_dest
            )
...
def get_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Porechop: a tool for finding adapters in Oxford "
    ...
    main_group.add_argument(
        "--format",
        choices=["auto", "fasta", "fastq", "fasta.gz", "fastq.gz"],
        default="auto",
    ...
    barcode_group = parser.add_argument_group(
    ...
    adapter_search_group.add_argument(
        "--adapters_file",
        type=str,
        help="Path to a JSON file containing adapter sequences",
        required=True,
    )

    end_trim_group = parser.add_argument_group(
        "End adapter settings", "Control the trimming of adapters from read ends"
    )
    end_trim_group.add_argument(
        "--end_size",
        type=int,
        default=150,
    ...
    middle_trim_group = parser.add_argument_group(
        "Middle adapter settings",
        "Control the splitting of read from middle " "adapters",
    )
    middle_trim_group.add_argument(
        "--no_split",
        action="store_true",
...
def load_adapters_from_file(adapters_file):
    my_adapters = json.loads(open(adapters_file).read())
    ADAPTERS_LIST = []
    for bc in my_adapters:
        ADAPTERS_LIST.append(
            Adapter(
                name=bc["name"],
                start_sequence=bc["start_sequence"],
                end_sequence=bc["end_sequence"],
            )
        )
...
def find_matching_adapter_sets(
    check_reads,
    verbosity,
    end_size,
    scoring_scheme_vals,
    ...
    if verbosity > 0:
        print(
            bold_underline("Looking for known adapter sets"),
            flush=True,
            file=print_dest,
        )
            ...
            for _ in pool.imap(align_adapter_set_one_arg, arg_list):
                ...
                if verbosity > 0 and finished_count % search_adapter_count == 0:
                    output_progress_line(
                        finished_count // search_adapter_count, read_count, print_dest
                    )
    ...
    return [
        x for x in search_adapters if x.best_start_or_end_score() >= adapter_threshold
    ]
...
def choose_barcoding_kit(adapter_sets, verbosity, print_dest):
    ...
    if verbosity > 0:
        print(
            "\nBarcodes determined to be in " + orientation + " orientation",
            file=print_dest,
        )
    return orientation
...
def fix_up_1d2_sets(matching_sets):
    ...
    if (
        "1D^2 part 1" in matching_set_names
        and "1D^2 part 2" in matching_set_names
        and "SQK-MAP006 Short" in matching_set_names
    ):
        part_1_score = [x for x in matching_sets if x.name == "1D^2 part 1"][
            0
        ].best_start_or_end_score()
        part_2_score = [x for x in matching_sets if x.name == "1D^2 part 2"][
            0
        ].best_start_or_end_score()
        sqk_score = [x for x in matching_sets if x.name == "SQK-MAP006 Short"][
            0
        ].best_start_or_end_score()
...
def find_adapters_at_read_ends(
    reads,
    matching_sets,
    verbosity,
    end_size,
    ...
    if verbosity > 0:
        ...
        name_len = max(
            max(
                len(x.start_sequence[0]) if x.start_sequence else 0
                for x in matching_sets
            ),
        ...
        for matching_set in matching_sets:
            if matching_set.start_sequence:
                print(
                    "  "
                    + matching_set.start_sequence[0].rjust(name_len)
                    + ": "
                    + red(matching_set.start_sequence[1]),
                    file=print_dest,
                )
            if matching_set.end_sequence:
                print(
                    "  "
                    + matching_set.end_sequence[0].rjust(name_len)
                    + ": "
                    + red(matching_set.end_sequence[1]),
                    file=print_dest,
                )
    ...
    if threads == 1:
        for read_num, read in enumerate(reads):
            ...
            if verbosity == 1:
                ...
                print(
                    read.formatted_start_and_end_seq(
                        end_size, extra_trim_size, check_barcodes
                    ),
                    file=print_dest,
                )
            elif verbosity > 2:
                print(
                    read.full_start_end_output(
                        end_size, extra_trim_size, check_barcodes
                    ),
                    file=print_dest,
                )
...
def display_read_end_trimming_summary(reads, verbosity, print_dest):
    ...
    print(
        int_to_str(start_trim_count).rjust(len(int_to_str(len(reads))))
        + " / "
        + int_to_str(len(reads))
        + " reads had adapters trimmed from their start ("
        + int_to_str(start_trim_total)
        + " bp removed)",
        file=print_dest,
    )
    print(
        int_to_str(end_trim_count).rjust(len(int_to_str(len(reads))))
        + " / "
        + int_to_str(len(reads))
        + " reads had adapters trimmed from their end ("
        + int_to_str(end_trim_total)
        + " bp removed)",
        file=print_dest,
    )
    print("\n", file=print_dest)
...
def find_adapters_in_read_middles(
    reads,
    matching_sets,
    verbosity,
    middle_threshold,
    extra_trim_good_side,
    extra_trim_bad_side,
    scoring_scheme_vals,
    print_dest,
    threads,
    ...
    if verbosity > 0:
        ...
        print(
            bold_underline(verb + " reads containing middle adapters"), file=print_dest
        )
    ...
    if threads == 1:
        for read_num, read in enumerate(reads):
            ...
            if read.middle_adapter_positions and verbosity > 1:
                print(
                    read.middle_adapter_results(verbosity), file=print_dest, flush=True
                )
...
def output_reads(
    ...
    if verbosity > 0:
        ...
        if barcode_dir is not None:
            ...
            destination = "file"
        print(
            bold_underline(verb + trimmed_or_untrimmed + " reads to " + destination),
            flush=True,
            file=print_dest,
        )
    ...
    if barcode_dir is not None:
        ...
        for read in reads:
            ...
            if barcode_name not in barcode_files:
                barcode_files[barcode_name] = open(
                    os.path.join(barcode_dir, barcode_name + "." + out_format), "wt"
                )
        ...
        for barcode_name in sorted(barcode_files.keys()):
            ...
            table_row = [
                barcode_name,
                int_to_str(barcode_read_counts[barcode_name]),
        ...
        if verbosity > 0:
            print("")
            print_table(
                table, print_dest, alignments="LRRL", max_col_width=60, col_separation=2
            )
    ...
    # Output to all reads to stdout.
        ...
        for read in reads:
            read_str = (
                read.get_fasta(min_split_size, discard_middle)
                if out_format == "fasta"
    ...
    # Output to all reads to file.
        ...
        if gzipped_out:
            subprocess.check_output(
                gzip_command + " -c " + out_filename + " > " + output,
                stderr=subprocess.STDOUT,
                shell=True,
            )
